...
<CodeTrace
  ...
  steps={[
    { line: 1, label: "periodic() is called — call #1 begins", state: { call: 1, count: "–" } },
    { line: 2, label: "int count = 0 — a brand new variable, created fresh this call", state: { call: 1, count: 0 } },
    { line: 3, label: "sensor.hasGamePiece() → true", state: { call: 1, count: 0 } },
    { line: 4, label: "count = count + 1 → count is now 1", state: { call: 1, count: 1 } },
    { line: 5, label: "println prints: \"Collected: 1\"", state: { call: 1, count: 1 } },
    { line: 7, label: "method returns — count is destroyed", state: { call: 1, count: "gone" }, comment: "count ceases to exist" },
    { line: 1, label: "periodic() is called again — call #2 begins", state: { call: 2, count: "–" } },
    ...
    { line: 5, label: "println prints: \"Collected: 1\" — again", state: { call: 2, count: 1 }, comment: "stuck at 1 forever" },
  ]}
/>
...
void periodic() {
  ...
  steps={[
    { line: 1, label: "count = 0 — declared once when the object is created", state: { call: 1, count: 0 }, comment: "runs at startup, not per call" },
    ...
    { line: 6, label: "println prints: \"Collected: 1\"", state: { call: 1, count: 1 } },
    { line: 7, label: "method returns — but count field survives", state: { call: 2, count: 1 }, comment: "count lives on" },
    ...
    { line: 6, label: "println prints: \"Collected: 2\"", state: { call: 2, count: 2 }, comment: "accumulates correctly" },
  ]}
/>
...
void periodic() {
  ...
  variables={{ call: 1, speed: 0.5, boost: "–" }}
  steps={[
    { line: 4, label: "boost = 0.0 — reset to zero every call", state: { call: 1, speed: 0.5, boost: 0.0 } },
