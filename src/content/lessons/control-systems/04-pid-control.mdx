...
## Simulation: PID flywheel controller

<RobotSimEditor
  client:load
  type="flywheel"
  starterCode={`public class PIDController {
    ...
    public static void periodic() {
        ...
        integrator += error * dt;
        double dTerm = first ? 0.0 : kD * (error - lastError) / dt;
        double output = kP * error + kI * integrator + dTerm;
  ...
  hint="Try turning off kI (set to 0) and observe steady-state error. Re-enable kI to eliminate it. Then try kD=0.01 to see if D helps or hurts. Note that PID-only (no feedforward) requires high gains to reach the setpoint, which risks instability."
/>
