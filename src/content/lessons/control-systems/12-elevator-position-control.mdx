...
## Simulation: elevator position controller

<RobotSimEditor
  client:load
  type="elevator"
  starterCode={`public class ElevatorController {
    // kG holds the elevator against gravity (constant, unlike arm)
    static double kG  = 0.7;
    static double kP  = 8.0;
    static double setpointM = 1.0;

    public static void periodic() {
        double posM  = getPositionMeters();
        double error = setpointM - posM;

        double ffVolts  = kG;          // constant gravity hold
        double pidVolts = kP * error;
        double total    = ffVolts + pidVolts;

        setVoltage(total);
        SmartDashboard.putNumber("position", posM);
        SmartDashboard.putNumber("setpoint", setpointM);
        SmartDashboard.putNumber("voltage",  total);
    }
}`}
  hint="Unlike the arm, gravity feedforward for an elevator is constant (kG doesn't change with height). Try setting kG=0 and observe how much harder PID has to work to hold position. Try setpointM=0.5 vs 1.2 to see the carriage move to different heights."
/>
