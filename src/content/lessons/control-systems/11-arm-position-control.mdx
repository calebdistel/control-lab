...
## Simulation: arm position controller

<RobotSimEditor
  client:load
  type="arm"
  starterCode={`public class ArmController {
    // Gravity feedforward: kG * cos(angle) compensates gravity
    ...
    public static void periodic() {
        ...
        double ffVolts = kG * Math.cos(rad);
  ...
  hint="Try setting kG=0 and observe how the arm struggles to hold position — especially below horizontal where gravity is strongest. Restore kG=0.65 and notice how the feedforward handles gravity, leaving PID to only correct small errors."
/>
