...
## Simulation: tune a P-only arm controller

<RobotSimEditor
  client:load
  type="arm"
  starterCode={`public class ArmController {
    // Try increasing kP to speed up response.
    // Watch for oscillation when kP is too high.
    static double kP = 0.05;
    static double setpointDeg = 90.0;

    public static void periodic() {
        double angleDeg = getAngleDeg();
        double error    = setpointDeg - angleDeg;
  ...
  hint="Notice the arm never quite reaches 90° — that gap is steady-state error caused by gravity. Try kP=0.05, 0.15, 0.5, 2.0 and observe the tradeoff: higher kP reduces steady-state error but eventually causes oscillation."
/>
