...
import RobotSimEditor from '../../../components/demos/RobotSimEditor.jsx';
...
## Simulation: FF + PID flywheel controller

<RobotSimEditor
  client:load
  type="flywheel"
  starterCode={`public class FlywheelController {
    // kS overcomes static friction; kV converts RPM to voltage
    static double kS = 0.20;
    static double kV = 0.00210;  // V / RPM
    static double kP = 0.001;
    static double setpointRPM = 3000.0;

    public static void periodic() {
        double rpm   = getRPM();
        double error = setpointRPM - rpm;

        double ffVolts  = kS + kV * setpointRPM;
        double pidVolts = kP * error;
        double total    = ffVolts + pidVolts;

        setVoltage(total);
        SmartDashboard.putNumber("rpm",      rpm);
        SmartDashboard.putNumber("setpoint", setpointRPM);
        SmartDashboard.putNumber("ff",       ffVolts);
        SmartDashboard.putNumber("pid",      pidVolts);
    }
}`}
  hint="Compare this to the PID-only simulation in the previous lesson. The FF alone gets the flywheel very close to the setpoint without needing large gains. Try setting kP=0 — notice how well the feedforward alone holds the speed. Then re-enable kP and see how quickly it corrects."
/>
